...
from utils.preprocess_data import (
    ...
    softmax,
    preprocess_text,
    combine_extra_request_fields,
    preprocess_tabular,
    preprocess_multimodal,
    temp_preprocess_fake_model,
)
...
class ImageClassifyService(BaseService):
    ...
    def predict_proba(self, data):
        ...
        _, logits = self.ort_sess.run(
            None, {self.input_names[0]: image_tensor, self.input_names[1]: valid_nums}
        )
    ...
    async def explain(self, params: ImageExplainRequest) -> dict:
        ...
        try:
            explainer = ImageExplainer(
                params.method, self.ort_sess, num_samples=100, batch_size=32
            )
            load_time = perf_counter() - start_load
    ...
    def predict_proba(self, data):
        ...
        _, logits = self.ort_sess.run(
            None,
            {
                ...
                self.input_names[1]: segment_ids,
                self.input_names[2]: valid_length,
            },
        )
        predictions = softmax(logits)
        print(len(predictions))
    ...
    async def explain(self, params: TextExplainRequest) -> dict:
        ...
        try:
            explainer = TextExplainer(
                params.method,
                self.ort_sess,
                class_names=self.model_metadata["class_names"],
            )
            load_time = perf_counter() - start_load
            inference_start = perf_counter()
...
class TabularClassifyService(BaseService):
    ...
    def predict_proba(self, data):
        predictions = self.ort_sess.predict_proba(
    ...
    async def explain(self, params: TabularExplainRequest) -> dict:
        ...
        try:
            explainer = TabularExplainer(
                params.method,
                self.ort_sess,
                class_names=self.ort_sess.class_labels,
                num_samples=100,
...
class MultiModalClassifyService(BaseService):
    ...
    async def predict(self, params: TabularPredictionRequest) -> dict:
        ...
        predictions = []
        try:
            data = pd.read_csv(params.tab_file_path, on_bad_lines="skip")

            if params.runName != "ag-20240923_164206--hq-bert-base-uncased":
                data = preprocess_multimodal(data)
            else:
                data = temp_preprocess_fake_model(data)
...
class InferenceService:
    ...
    async def predict(self, req: BaseRequest) -> dict:
        ...
        match (req.task):
            ...
            case "TEXT_CLASSIFICATION":
                res = await self.text_classify_service.predict(
                    combined_params["params"]
                )
            case "TABULAR_CLASSIFICATION":
                res = await self.tabular_classify_service.predict(
                    combined_params["params"]
                )
            case "MULTIMODAL_CLASSIFICATION":
                res = await self.multimodal_classify_service.predict(
                    combined_params["params"]
                )
    ...
    async def explain(self, req: BaseRequest) -> dict:
        ...
        match (req.task):
            ...
            case "TEXT_CLASSIFICATION":
                res = await self.text_classify_service.explain(
                    combined_params["params"]
                )
            case "TABULAR_CLASSIFICATION":
                res = await self.tabular_classify_service.explain(
                    combined_params["params"]
                )
            case "MULTIMODAL_CLASSIFICATION":
                res = await self.multimodal_classify_service.explain(
                    combined_params["params"]
                )
        return res
