...
class AutogluonTrainer(object):
    ...
    def parse_args(self, kwargs: Optional[dict] = None):
        ...
        self.fit_args['hyperparameters'] = get_hyperparameter_config('light' if not self.has_special_types else 'multimodal')
            #'presets': 'best_quality',
            #'hyperparameters': get_hyperparameter_config('light' if not self.has_special_types else 'multimodal'),
    ...
    async def train_async(self, label:str, train_data_path: Path, val_data_path: Path, model_path: Path) -> Union[
        TabularPredictor, None]:
